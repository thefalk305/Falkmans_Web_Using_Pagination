...
const departingGothenburg =  "<b>Göteborg</b><br>Departing Gothenburg, Sweden.  He set sail for Kingston on Hull on the 9th upon the S S Romeo, a steamship built and owned by the Wilson Steamship Line "
...
const arrivalChicago  = `A city with the representation of literally hundreds of ethnic groups, Chicago has rightfully earned its nickname as "The Melting Pot of America". This is where, in the early 1890's, Nils Falkman and his family finally settled. Here is were they set down their roots. Many generations of Falkman's were born here and still live in and around the Chicagoland area.`
  ...
  const landRoute0 = [
      [56.57, 13.55], [56.9, 12.2], [57.2, 12.2], [57.70, 11.90]
  ];
  ...
  const segmentTypes = [
    ...Array(landRoute0.length).fill('train'),
...
const movingMarker = L.marker(allPoints[0], {
  ...
  function moveMarker() {
    ...
    const icon = L.divIcon({
      html: iconType === 'boat' ? '<div style="font-size: 32px;" >🚢</div>' : '<div style="font-size: 32px;" >🚂</div>',
      iconSize: [0, 0],
      iconAnchor: [16, 16]
    });
    movingMarker.setLatLng(allPoints[currentIndex]);
